#
DB_PATH = "/home/nguyen/Desktop/Formula_DB"
...
def check_config():
    ...
    if MODE == "generate":
        list_get_data_func = [getattr(gf, key) for key in db_fields.keys()]
        list_db_field = []
        for key in db_fields.keys():
            list_db_field += db_fields[key]
    elif MODE == "update":
        list_key = list(db_fields.keys())
        list_get_data_func = [getattr(gf, key) for key in list_key[-N_UPDATE:]]
        list_db_field = []
